...
// stop_teamdesk_servers() is just a temp solution here.
fn force_stop_server() {
    stop_teamdesk_servers();
...
mod desktop {
    ...
    pub struct Desktop {
        ...
        pub home: String,
        pub dbus: String,
        pub is_teamdesk_subprocess: bool,
    ...
    impl Desktop {
        ...
        pub fn is_headless(&self) -> bool {
            self.sid.is_empty() || self.is_teamdesk_subprocess
        }
        ...
        fn set_is_subprocess(&mut self) {
            self.is_teamdesk_subprocess = false;
            let cmd = format!(
            ...
            if let Ok(res) = run_cmds(&cmd) {
                if res.trim() != "0" {
                    self.is_teamdesk_subprocess = true;
                }
            }
        }
...
pub fn uninstall_service(show_new_window: bool, _: bool) -> bool {
    ...
    // systemctl kill teamdesk --tray, execute cp first
...
fn get_shortcuts_inhibitor_app_id() -> String {
    if is_flatpak() {
        ...
        // (e.g., "com.teamdesk.TeamDesk"). This is the most reliable source.
