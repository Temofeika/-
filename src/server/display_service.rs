...
pub(super) fn get_original_resolution(
    ...
    #[cfg(windows)]
    let is_teamdesk_virtual_display =
        crate::virtual_display_manager::teamdesk_idd::is_virtual_display(&display_name);
    #[cfg(not(windows))]
    let is_teamdesk_virtual_display = false;
    Some(if is_teamdesk_virtual_display {
        Resolution {
            width: 0,
            height: 0,
            ..Default::default()
