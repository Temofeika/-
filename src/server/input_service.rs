...
impl VirtualInputState {
    fn new() -> Option<Self> {
        VirtualInput::new(
            ...
            // 1. Press "Command" key in TeamDesk
            // 2. Move the physical mouse
            // 3. Press "V" key in TeamDesk
            ...
            // we consider only TeamDesk operates the controlled side.
            //
...
#[cfg(target_os = "macos")]
fn key_sleep() {
    // https://www.reddit.com/r/teamdesk/comments/1kn1w5x/typing_lags_when_connecting_to_macos_clients/
    //
    ...
    // Though `/Applications/TeamDesk.app/Contents/MacOS/teamdesk --server` in terminal is ok.
    let now = Instant::now();
...
pub async fn lock_screen() {
    ...
    if #[cfg(target_os = "linux")] {
        ...
        // loginctl lock-session also not work, they both work run teamdesk from cmd
        std::thread::spawn(|| {
