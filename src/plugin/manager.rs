...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "teamdesk".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/teamdesk-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "TeamDesk wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "TeamDesk wants to uninstall the plugin",
    )
}
