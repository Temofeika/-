...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/teamdesk')
    system2('mkdir -p tmpdeb/etc/teamdesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/teamdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/teamdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/teamdesk/')
    system2(
    ...
    system2(
        'cp ../res/teamdesk-link.desktop tmpdeb/usr/share/applications/teamdesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/teamdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/teamdesk/')
    system2(
        'cp ../res/pam.d/teamdesk.debian tmpdeb/etc/pam.d/teamdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/teamdesk/files/polkit && chmod a+x tmpdeb/usr/share/teamdesk/files/polkit")

    system2('mkdir -p tmpdeb/DEBIAN')
    generate_control_file(version)
    system2('cp -a ../res/DEBIAN/* tmpdeb/DEBIAN/')
    md5_file_folder("tmpdeb/")
    system2('dpkg-deb -b tmpdeb teamdesk.deb;')
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/teamdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/teamdesk/')
    system2(
        'cp ../res/teamdesk.service tmpdeb/usr/share/teamdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/teamdesk.png')
...
def build_flutter_dmg(version, features):
    ...
    # copy dylib
    system2(
        "cp target/release/liblibteamdesk.dylib target/release/libteamdesk.dylib")
    os.chdir('flutter')
...
def build_flutter_arch_manjaro(version, features):
    ...
    os.chdir('flutter')
    system2('flutter build linux --release')
    system2(f'strip {flutter_build_dir}/lib/libteamdesk.so')
    os.chdir('../res')
...
def build_flutter_windows(version, features, skip_portable_pack):
    if not skip_cargo:
        system2(f'cargo build --features {features} --lib --release')
        if not os.path.exists("target/release/libteamdesk.dll"):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/teamdesk.exe')
    os.chdir('../..')
    if os.path.exists('./teamdesk_portable.exe'):
        os.replace('./target/release/teamdesk-portable-packer.exe',
                   './teamdesk_portable.exe')
    else:
        os.rename('./target/release/teamdesk-portable-packer.exe',
                  './teamdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/teamdesk_portable.exe')
    os.rename('./teamdesk_portable.exe', f'./teamdesk-{version}-install.exe')
    print(
...
def main():
    ...
    if windows:
        ...
        if flutter:
            if osx:
                ...
                # system2(
                #     'mv target/release/bundle/deb/teamdesk*.deb ./flutter/teamdesk.deb')
                build_flutter_deb(version, features)
        else:
            system2('cargo bundle --release --features ' + features)
            if osx:
                system2(
                    'strip target/release/bundle/osx/TeamDesk.app/Contents/MacOS/teamdesk')
                system2(
                    'cp libsciter.dylib target/release/bundle/osx/TeamDesk.app/Contents/MacOS/')
    ...
    #rcodesign sign --p12-file ~/.p12/teamdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/TeamDesk.app/Contents/MacOS/teamdesk
    #rcodesign sign --p12-file ~/.p12/teamdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/TeamDesk.app/Contents/MacOS/libsciter.dylib
    #rcodesign sign --p12-file ~/.p12/teamdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./target/release/bundle/osx/TeamDesk.app
    # goto "Keychain Access" -> "My Certificates" for below id which starts with "Developer ID Application:"
    codesign -s "Developer ID Application: {0}" --force --options runtime  ./target/release/bundle/osx/TeamDesk.app/Contents/MacOS/*
    ...
    #rcodesign sign --p12-file ~/.p12/teamdesk-developer-id.p12 --p12-password-file ~/.p12/.cert-pass --code-signature-flags runtime ./teamdesk-{1}.dmg
    codesign -s "Developer ID Application: {0}" --force --options runtime ./teamdesk-{1}.dmg
    ...
    '''.format(pa, version))
                ...
                # build deb package
                system2(
                    'mv target/release/bundle/deb/teamdesk*.deb ./teamdesk.deb')
                system2('dpkg-deb -R teamdesk.deb tmpdeb')
                system2('mkdir -p tmpdeb/usr/share/teamdesk/files/systemd/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
                system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
                system2(
                ...
                system2(
                    'cp res/teamdesk-link.desktop tmpdeb/usr/share/applications/teamdesk-link.desktop')
                os.system('mkdir -p tmpdeb/etc/teamdesk/')
                os.system('cp -a res/startwm.sh tmpdeb/etc/teamdesk/')
                os.system('mkdir -p tmpdeb/etc/X11/teamdesk/')
                os.system('cp res/xorg.conf tmpdeb/etc/X11/teamdesk/')
                os.system('cp -a DEBIAN/* tmpdeb/DEBIAN/')
                os.system('mkdir -p tmpdeb/etc/pam.d/')
                os.system('cp pam.d/teamdesk.debian tmpdeb/etc/pam.d/teamdesk')
                system2('strip tmpdeb/usr/bin/teamdesk')
                system2('mkdir -p tmpdeb/usr/share/teamdesk')
                system2('mv tmpdeb/usr/bin/teamdesk tmpdeb/usr/share/teamdesk/')
